...
export default function DarkModeIconButton() {
  const isBrowsertDark = () =>
    window.matchMedia("(prefers-color-scheme: dark)").matches;

  const isDarkDefaultTheme = (): boolean => {
    const localStorageTheme = localStorage.getItem("theme");
    const browserDefault = isBrowsertDark() ? "dark" : "light";

    if ((localStorageTheme || browserDefault) === "dark") {
      return true;
    }
    return false;
  };

  const [isDark, setIsDark] = useState<boolean>(isDarkDefaultTheme());
  ...
  return (
    <button
      ...
      type="button"
      className="group relative rounded-full p-1 transition ease-in-out hover:fill-white hover:outline-none hover:ring-2 hover:ring-offset-2"
    >
      {isDark ? (
        <svg
          className="h-5 w-5 fill-blue-wild group-hover:fill-white"
        ...
        <svg
          className="h-5 w-5 fill-blue-wild hover:animate-pulse group-hover:fill-white"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
